...
/// Semaphore for directory thumbnail generation (the collage/sprite build).
/// Kept separate from THUMB_LIMITER: directory generation is heavier (multiple
/// images + ImageMagick) but can run 2 in parallel without saturating the CPU.
pub static DIR_THUMB_LIMITER: tokio::sync::Semaphore = tokio::sync::Semaphore::const_new(2);
