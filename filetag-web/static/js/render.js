...
function renderGrid(items) {
    ...
    for (const entry of items) {
        ...
        if (isDir) {
            ...
            preview = `<div class="card-icon" data-thumb-src="/thumb/${encodeURI(fullPath(entry))}${dirParam('?')}" data-name="${esc(name)}" data-thumb-hover="1">${fileIcon(name)}</div>`;
        } else if (type_ === 'video') {
            ...
            preview = `<div class="card-icon" data-thumb-src="/thumb/${encodeURI(fullPath(entry))}${dirParam('?')}" data-name="${esc(name)}" data-thumb-hover="1">${ICONS.zip || ''}</div>` +
                `<div class="card-filmstrip-badge">${ICONS.zip || ''}</div>`;
        } else if (type_ === 'pdf') {
            preview = `<div class="card-icon" data-thumb-src="/thumb/${encodeURI(fullPath(entry))}${dirParam('?')}" data-name="${esc(name)}" data-thumb-hover="1">${ICONS.pdf}</div>` +
